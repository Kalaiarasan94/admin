...
const Machines: React.FC = () => {
    ...
    const getStatusColor = (status: string) => {
        switch (status.toLowerCase()) {
            case 'running': return 'text-emerald-500 bg-emerald-50 dark:bg-emerald-900/20';
            case 'paused': return 'text-amber-500 bg-amber-50 dark:bg-amber-900/20';
            case 'idle': return 'text-slate-500 bg-slate-50 dark:bg-slate-900/20';
            case 'repair': return 'text-rose-500 bg-rose-50 dark:bg-rose-900/20';
        ...
        <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 text-xs font-black uppercase tracking-[0.2em] mb-2">
                        <Activity size={14} /> Production Line
                    </div>
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Machine Fleet</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Real-time status monitoring and lifecycle management.</p>
                </div>
                <button 
                    onClick={() => handleOpenModal()}
                    className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3.5 rounded-[1.5rem] font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-emerald-500/25 active:scale-95"
                >
                    <Plus size={18} />
                    Commission Unit
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <AnimatePresence mode="popLayout">
                    {loading ? (
                        [1, 2, 3].map(i => (
                            <div key={i} className="h-[300px] bg-slate-100 dark:bg-slate-800/50 rounded-[2.5rem] animate-pulse" />
                        ))
                    ) : machines.length === 0 ? (
                        ...
                        machines.map((machine, idx) => (
                            <motion.div 
                                ...
                                key={machine.id}
                                className="glass rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-soft group overflow-hidden"
                            >
                                <div className="p-8 space-y-6">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                                            <button 
                                                ...
                                                className="p-2.5 bg-white dark:bg-slate-800 text-slate-400 hover:text-blue-600 rounded-xl shadow-soft border border-slate-100 dark:border-slate-700 transition-all active:scale-90"
                                            >
                                                <Edit2 size={14} />
                                            </button>
                                            <button 
                                                onClick={() => handleDelete(machine.id)}
                                                className="p-2.5 bg-white dark:bg-slate-800 text-slate-400 hover:text-rose-600 rounded-xl shadow-soft border border-slate-100 dark:border-slate-700 transition-all active:scale-90"
                                            >
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{machine.machine_name}</h3>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 pt-2">
                                        <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl">
                                            ...
                                            <p className="text-sm font-black text-slate-700 dark:text-slate-300">{machine.total_output || 0} Units</p>
                                        </div>
                                        <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl">
                                            <div className="flex items-center gap-2 text-slate-400 mb-1">
                                                <Clock size={12} />
                                            ...
                                            <p className="text-sm font-black text-slate-700 dark:text-slate-300">{machine.shift || 'N/A'}</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="px-8 py-5 bg-slate-50/50 dark:bg-slate-800/30 border-t border-slate-50 dark:border-slate-800 flex justify-between items-center">
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="p-10 space-y-6">
                                <div className="space-y-6">
                                    <div className="group">
                                        ...
                                        <div className="relative">
                                            <Settings className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-500 transition-colors" size={18} />
                                            <input 
                                                type="text" 
                                                value={name}
                                                onChange={(e) => setName(e.target.value)}
                                                className="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-[1.5rem] text-sm font-bold text-slate-900 dark:text-white focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all"
                                                required
                                    ...
                                    <div className="group">
                                        ...
                                        <div className="relative">
                                            <Power className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-500 transition-colors" size={18} />
                                            <select 
                                                value={status}
                                                onChange={(e) => setStatus(e.target.value)}
                                                className="w-full pl-14 pr-10 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-[1.5rem] text-sm font-bold text-slate-900 dark:text-white focus:ring-4 focus:ring-emerald-500/10 outline-none appearance-none cursor-pointer"
                                            >
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="group">
                                            ...
                                            <input 
                                                ...
                                                className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800 border-none rounded-[1.5rem] text-sm font-bold text-slate-900 dark:text-white focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all"
                                                required
                                            />
                                        </div>
                                        <div className="group">
                                ...
                                <div className="pt-6 flex gap-4">
                                    ...
                                    <button 
                                        type="submit"
                                        className="flex-[2] px-6 py-4 text-xs font-black uppercase tracking-widest text-white bg-emerald-600 hover:bg-emerald-700 rounded-2xl transition-all shadow-lg shadow-emerald-500/25 active:scale-95"
                                    >
