...
const SalesApprovals: React.FC = () => {
    ...
    const handleDownload = (id: number) => {
        // Points to the main backend server (3000) for PDF generation
        const url = `http://${window.location.hostname}:3000/sales/invoices/${id}/pdf?isAdmin=true`;
    ...
    const getStatusConfig = (status: string) => {
        switch (status) {
            ...
            case 'rejected':
                return { color: 'text-rose-600 bg-rose-50 dark:bg-rose-900/20 border-rose-100 dark:border-rose-900/30', icon: XCircle, label: 'Rejected' };
            default:
        ...
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700 relative">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-1">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Sales Approvals</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Verify and authorize sales invoices before distribution.</p>
                </div>
            </header>
                            ...
                            return (
                                ...
                                >
                                    <div className="absolute top-0 right-0 p-6 flex gap-2">
                                        <button onClick={() => handleDelete(inv.id)} className="p-3 bg-white dark:bg-slate-800 text-slate-400 hover:text-rose-600 rounded-xl shadow-soft transition-all active:scale-90 opacity-0 group-hover:opacity-100">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex-1 space-y-6">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight mb-1 group-hover:text-blue-600 transition-colors line-clamp-1">
                                                {inv.customer_name}
                                            </h3>
                                        ...
                                        <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl">
                                            ...
                                            <p className="text-2xl font-black text-slate-900 dark:text-white">₹{inv.total_amount}</p>
                                        </div>
                                    </div>
                                        ...
                                        {inv.approval_status === 'pending' ? (
                                            <>
                                                ...
                                                <button 
                                                    ...
                                                    className="px-4 py-3.5 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-rose-500 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-rose-50 transition-all active:scale-95"
                                                >
                                                    Reject
                                                </button>
                                            </>
                                            ...
                                            <>
                                                <button 
                                                    ...
                                                    className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl transition-all active:scale-95"
                                                >
                                                    <Download size={14} />
                                                    Download PDF
                                                </button>
                                                {inv.approval_status === 'rejected' && (
                                                    <button 
                                                        ...
                                                        className="px-4 py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-2xl text-xs font-black uppercase tracking-widest hover:text-blue-600 transition-all"
                                                    >
                                                        Review
                                                    </button>
                                                )}
