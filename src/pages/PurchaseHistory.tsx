        ...
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700 relative">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-1">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Purchase History</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">View and audit all completed and approved material purchases.</p>
                </div>
            </header>
                            ...
                            {loading ? (
                                [1, 2, 3, 4, 5].map(i => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan={6} className="px-8 py-6 h-20 bg-slate-50/50 dark:bg-slate-800/10" />
                                    </tr>
                                ))
                            ) : error ? (
                                <tr>
                                    ...
                                    >
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                <div className="p-3 bg-blue-50 dark:bg-blue-900/10 rounded-2xl text-blue-600 dark:text-blue-400">
                                                    <Package size={18} />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{order.material_name}</p>
                                        ...
                                        <td className="px-8 py-6 text-center">
                                            <span className="text-sm font-black text-slate-700 dark:text-slate-300">{order.purchased_quantity}</span>
                                        </td>
                                        <td className="px-8 py-6 text-center">
                                            <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">₹{parseFloat(order.price).toFixed(2)}</span>
                                        </td>
                                        <td className="px-8 py-6">
                                            <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">{order.vendor_name}</span>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/30 rounded-lg text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest">
                                                <CheckCircle size={10} />
                                                Completed
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <button 
                                                ...
                                                className="p-3 text-slate-400 hover:bg-white dark:hover:bg-slate-800 hover:text-blue-600 hover:shadow-soft rounded-xl transition-all active:scale-90"
