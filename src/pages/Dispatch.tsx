...
const Dispatch: React.FC = () => {
    ...
    const handleDownloadPDF = (pdfPath: string) => {
        if (!pdfPath) return;
        const url = `http://${window.location.hostname}:3000${pdfPath}`;
        ...
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700 relative">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-1">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Dispatch History</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Review and download all dispatch reports and delivery notes.</p>
                </div>
            </header>
                                    ...
                                    >
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">DISPATCH #{item.id}</p>
                                                    ...
                                                    <p className="text-[10px] text-blue-500 font-bold mt-1">{item.items_summary}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-8 py-6">
                                            <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">{item.customer_name}</span>
                                        </td>
                                        <td className="px-8 py-6">
                                            <span className="text-sm font-black text-slate-700 dark:text-slate-300">{item.dispatched_by_name}</span>
                                        </td>
