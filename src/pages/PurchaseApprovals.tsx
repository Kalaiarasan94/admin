...
const PurchaseApprovals: React.FC = () => {
    ...
    const handleDownloadPDF = (id: number) => {
        const url = `http://${window.location.hostname}:3000/purchase/orders/${id}/pdf?isAdmin=true`;
        ...
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700 relative">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-1">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Purchase Order Approvals</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Verify and authorize final purchase orders from Accounts.</p>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <AnimatePresence mode="popLayout">
                    {loading ? (
                        [1, 2, 3].map(i => (
                            <div key={i} className="h-[350px] bg-slate-100 dark:bg-slate-800/50 rounded-[2.5rem] animate-pulse" />
                        ))
                    ) : error ? (
                        <div className="col-span-full py-20 text-center">
                            <div className="p-8 bg-rose-50 dark:bg-rose-900/10 rounded-3xl border border-rose-100 dark:border-rose-900/20 max-w-md mx-auto">
                                ...
                                <button onClick={fetchOrders} className="px-6 py-3 bg-slate-900 dark:bg-white dark:text-slate-900 text-white rounded-xl text-sm font-bold shadow-xl">Retry</button>
                            </div>
                        </div>
                        ...
                        filteredOrders.map((order, idx) => (
                            ...
                            >
                                <div className="flex-1 space-y-6">
                                    <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border text-amber-600 bg-amber-50 dark:bg-amber-900/20 border-amber-100 dark:border-amber-900/30`}>
                                        <Clock size={12} />
                                        Awaiting Admin Approval
                                    </div>

                                    <div>
                                        <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight mb-1 group-hover:text-indigo-600 transition-colors line-clamp-1">
                                            {order.material_name}
                                        </h3>
                                    ...
                                    <div className="pt-4 border-t border-slate-50 dark:border-slate-800/50 space-y-2">
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                                            ...
                                            <span className="text-slate-600 dark:text-slate-300">{order.created_by}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                                            <span className="text-slate-400">Request Ref:</span>
                                            <span className="text-slate-600 dark:text-slate-300">REQ #{order.request_id}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mt-8 space-y-3">
                                    <button 
                                        ...
                                        className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-indigo-500/20 transition-all active:scale-95"
                                    >
                                        Approve Purchase
                                    </button>
                                    <button 
                                        onClick={() => handleDownloadPDF(order.id)}
                                        className="w-full py-3.5 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95"
                                    >
                                        View Invoice
                                    </button>
                                </div>
